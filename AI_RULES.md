...
## 🎯 LANDING PAGE PROJECTS - IMPORTANT

For Landing Page projects:
1. **ALWAYS modify `src/app/page.tsx`** as the main landing page - DO NOT create new routes like `/pets` or `/about`
2. **Create components in `src/components/landing/`** for organization
3. **The landing page should be complete and functional** at the root URL (`/`)
4. **DO NOT create multiple pages** - Landing projects are single-page applications
5. **ALWAYS include "use client" directive** when using interactive elements

## 🚨 CRITICAL: Next.js App Router - Client vs Server Components

**IMPORTANT**: Next.js 13+ uses Server Components by default. You MUST add the `"use client"` directive at the top of the file when:

1. **Using Event Handlers** (`onClick`, `onChange`, `onSubmit`, `onFocus`, etc.)
2. **Using Browser APIs** (`window`, `document`, `localStorage`, `sessionStorage`, etc.)
3. **Using React Hooks** (`useState`, `useEffect`, `useContext`, `useReducer`, etc.)
4. **Using interactive libraries** (toast notifications, animations, client-only packages)

### Examples:

#### ❌ WRONG (will cause error):
```tsx
// Missing "use client" with onClick handler
...
#### ✅ CORRECT:
```tsx
"use client"; // Required for interactivity - ALWAYS first line
...
### Rule: ALWAYS add `"use client"` to:
- Landing pages with forms or CTAs
- Components with any interactivity
- Pages using `toast` from sonner
- Components with state management
- Any file with event handlers
